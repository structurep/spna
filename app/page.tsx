...
export default function Home() {
  ...
  return (
    <main className="flex min-h-screen flex-col px-6 pb-12 pt-8 md:px-12 lg:px-20">
      <div className="mx-auto flex w-full max-w-5xl flex-1 flex-col gap-16">
        <header
          className={`nav-shell sticky top-0 z-40 flex items-center justify-between border-b border-transparent ${scrolled ? "scrolled py-4" : "py-6"}`}
        >
          <Image
            src="/images/logo.svg"
        ...
        <section id="about" className="flex flex-col gap-12 pb-8 pt-4">
          <div className="max-w-3xl space-y-5">
            <p className="hero-line text-[11px] tracking-[0.22em] uppercase text-spmuted">
              Industrial &amp; field services M&amp;A
            </p>
            <h1 className="hero-line font-display text-[36px] font-medium leading-[1.12] text-sptext md:text-[44px] lg:text-[48px]">
          ...
          <section id="mandates" className="max-w-3xl space-y-4">
            ...
            <div className="grid gap-5 md:grid-cols-2">
              {mandates.map(item => (
                <div
                  key={item.label}
                  className="card-fade rounded-[26px] border border-[rgba(0,0,0,0.05)] bg-[#faf5ee] px-6 py-5 shadow-[0_18px_40px_rgba(0,0,0,0.04)] transition-transform duration-300 ease-out hover:-translate-y-1 hover:shadow-[0_26px_48px_rgba(0,0,0,0.08)] md:px-6 md:py-6"
                >
                  <p className="text-[10px] tracking-[0.2em] uppercase text-spmuted">
                    {item.label}
                  </p>
                  <p className="mt-3 text-[14px] leading-6 text-sptext/85">
                    {item.copy}
                  </p>
                </div>
              ))}
        ...
        <footer className="border-t border-spborder pt-10 text-[11px] text-spmuted">
